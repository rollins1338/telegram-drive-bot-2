...
def build_browser_keyboard(user_id, folders, files, total_items):
    ...
    if selected:
        ...
        # Upload button (merged from the duplicate buttons issue)
        selection_row.append(
            InlineKeyboardButton(f"📤 Upload ({selection_count})", callback_data="browser_download")
        )
        selection_row.append(
    ...
    utility_row.append(InlineKeyboardButton("⭐ Fav", callback_data="browser_favorites"))
        ...
        for idx, file_id in enumerate(file_ids, 1):
            ...
            try:
                ...
                while not done:
                    ...
                    # Update progress with visual bar and FloodWait protection
                    if status:
                        progress = int(status.progress() * 100)
...
async def upload_to_telegram_task(client, status_msg, folders, files, service):
    ...
    try:
        ...
        # Notify about any folder errors
        if folder_errors:
            error_note = "⚠️ Some folders had issues:\n" + "\n".join(f"• {err}" for err in folder_errors[:5])
            try:
                await status_msg.edit_text(
                    f"{error_note}\n\n"
                    f"📤 Proceeding with {len(all_files)} available files..."
                )
        ...
        for idx, file_meta in enumerate(all_files, 1):
            ...
            try:
                ...
                while not done:
                    ...
                    # Show progress bar for Drive download
                    if status:
                        progress = int(status.progress() * 100)
                        current_time = time.time()
